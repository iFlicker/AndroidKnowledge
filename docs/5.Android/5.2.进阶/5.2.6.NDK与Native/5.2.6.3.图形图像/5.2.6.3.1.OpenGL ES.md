...
```mermaid
graph TD
    subgraph CPU_Space ["CPU 内存空间"]
        VData["顶点数据 (坐标, 纹理坐标, 法线等)"]
        ShaderSource["着色器源码 (GLSL)"]
    end

    subgraph GPU_Space ["GPU 显存 / 统一内存"]
        VBO["顶点缓冲对象 (VBO)"]
        VAO["顶点数组对象 (VAO)"]
    ...
    subgraph GPU_Pipeline ["GPU 渲染流水线 (硬件管线)"]
        direction TB
        ...
        subgraph Per_Fragment ["逐片元操作 (Per-Fragment Operations)"]
...
*   **优化对策**：
    ...
    2.  **AHardwareBuffer 映射**：
        在 Android 8.0+（API 26，详见 [AndroidVersionChangeLog.md](../../../../../AndroidVersionChangeLog.md#android-80--81api-26--27)）上，利用 NDK 中的 `AHardwareBuffer` 直接建立与物理显存的共享页表映射，实现接近零拷贝的高速读取。
